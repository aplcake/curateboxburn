...
export function AdminPanel() {
  ...
  const handlePool = async (action: 'start' | 'stop') => {
    ...
    try {
      ...
      setMsg(action === 'start' ? '🎟️ Pool open — scanning every 30s' : '⏹ Pool closed');
      await refresh();
    } catch (e: unknown) { setMsg((e as Error).message); }
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-lg">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs text-white/60">
          ...
          <div className="stat-box col-span-2 flex justify-between items-center">
            ...
            <span className={status.poolOpen ? 'text-green-400' : status.poolBatchSent ? 'text-blue-400' : 'text-white/40'}>
              {status.poolBatchSent ? '● COMPLETE ✓' : status.poolOpen ? `● OPEN — ${status.poolCount}/${status.poolMax}` : `● CLOSED — ${status.poolCount ?? 0}/15`}
            </span>
          </div>
          <div className="stat-box col-span-2 flex justify-between items-center">
      ...
      <div className="flex flex-col gap-2 border border-white/10 p-4">
        ...
        <div className="flex gap-3">
          <button
            className="flex-1 py-2 border border-green-700/50 text-green-400 text-xs tracking-widest hover:bg-green-900/20 transition disabled:opacity-40"
            disabled={poolToggling || !!status?.poolOpen || !!status?.poolBatchSent}
            onClick={() => handlePool('start')}
          >▶ OPEN POOL</button>
          <button
            className="flex-1 py-2 border border-red-700/50 text-red-400 text-xs tracking-widest hover:bg-red-900/20 transition disabled:opacity-40"
            disabled={poolToggling || !status?.poolOpen}
            onClick={() => handlePool('stop')}
          >⏹ CLOSE POOL</button>
        </div>
        <p className="text-[10px] text-white/30 text-center">
          Scan every 30s. Accepted → instant Manifold NFT. Over 15 → auto-returned. Full → batch burn.
        </p>
      </div>
