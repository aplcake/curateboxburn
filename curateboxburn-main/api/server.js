...
async function returnToken(toAddress, amount) {
  const wc = getMinterClient();
  if (!wc) return;
  try {
    const hash = await wc.writeContract({
      address: TOKEN_CONTRACT, abi: ERC1155_TRANSFER_ABI, functionName: 'safeTransferFrom',
      args: [wc.account.address, toAddress, TOKEN_ID, BigInt(amount), '0x'],
    });
    console.log(`[pool] returned ${amount} token(s) to ${toAddress} — ${hash}`);
    return hash;
  } catch (err) { console.error(`[pool] return failed for ${toAddress}:`, err.message); }
}
...
async function sendBatchToDead(count) {
  ...
  try {
    const hash = await wc.writeContract({
      address: TOKEN_CONTRACT, abi: ERC1155_TRANSFER_ABI, functionName: 'safeTransferFrom',
      args: [wc.account.address, DEAD_ADDRESS, TOKEN_ID, BigInt(count), '0x'],
    });
    setPoolBatchSent();
    console.log(`[pool] batch-burned ${count} tokens — ${hash}`);
    if (DISCORD_WEBHOOK) {
...
async function scanPoolTransfers() {
  ...
  const wc = getMinterClient();
  if (!wc) return;
  const minterAddress = wc.account.address.toLowerCase();
  ...
  for (const log of relevant) {
    ...
    if (!fresh.poolOpen || poolFull || alreadyIn) {
      ...
      if (fresh.poolCount + 1 >= POOL_MAX) {
        console.log('[pool] full — batch burning in 3s');
        setPoolOpen(false);
        setTimeout(() => sendBatchToDead(POOL_MAX).catch(e => console.error('[pool] batch error:', e.message)), 3000);
      }
    }
  }
}
...
function startPoolScanner() {
  ...
  poolScannerInterval = setInterval(() => scanPoolTransfers().catch(e => console.error('[pool] scan error:', e.message)), 30_000);
